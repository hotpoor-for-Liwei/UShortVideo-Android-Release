...
extern "C" {
    ...
    JNIEXPORT jint JNICALL Java_com_sensetime_stmobile_STMobileStickerNative_preloadPackage(JNIEnv * env, jobject obj, jstring path);
    JNIEXPORT jint JNICALL Java_com_sensetime_stmobile_STMobileStickerNative_preloadPackageFromAssetsFile(JNIEnv * env, jobject obj, jstring file_path, jobject assetManager);
...
JNIEXPORT jint JNICALL Java_com_sensetime_stmobile_STMobileStickerNative_preloadPackage(JNIEnv * env, jobject obj, jstring path)
{
    ...
    if(stickerhandle != NULL) {
        result = st_mobile_sticker_preload_package(stickerhandle, pathChars);
    }
    if (pathChars != NULL) {
        env->ReleaseStringUTFChars(path, pathChars);
    }
...
JNIEXPORT jint JNICALL Java_com_sensetime_stmobile_STMobileStickerNative_preloadPackageFromAssetsFile(JNIEnv * env, jobject obj, jstring file_path, jobject assetManager)
{
    ...
    if(file_path == NULL){
        LOGE("preload sticker to null");
        return ST_JNI_ERROR_INVALIDARG;
    }

    const char* sticker_file_name_str = env->GetStringUTFChars(file_path, 0);
    if(NULL == sticker_file_name_str) {
        LOGE("file_name to c_str failed, preload sticker to null");
        return ST_JNI_ERROR_INVALIDARG;
    }
    ...
    if(stickerHandle != NULL) {
        result = st_mobile_sticker_preload_package_from_buffer(stickerHandle, buffer, size);
    }
    ...
    if(result != 0){
        LOGE("preload_package_from_buffer failed, %d",result);
        return result;
    }
